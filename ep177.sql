...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('177',     
                     'Ever ever more (pointless) reproccessing',
                     'Yeesh, but this is fiddly and expensive and... pointless');
...
from (select * from recipeTarget where recipe = 'angels-plate-titanium' and craftingName = 'Casting Machine MK3' and targetName = 'Red Belt 2 side') r1
...
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Furnace MK3' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Furnace MK3' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
...
where r2.recipe IN ('molten-titanium-smelting-1')
