...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('190',     
                     'A start on proper electronic logic boards',
                     'Too much procrastination, time to properly automate the logic boards..');
...
from (select * from recipeTarget where recipe = 'angels-plate-gold' and craftingName = 'Ore Processing Machine MK4' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Ore Processing Machine MK4' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Ore Processing Machine MK4' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Ore Processing Machine MK4' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Ore Processing Machine MK4' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Ore Processing Machine MK4' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
