...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('235',     
                     'More construction robots',
                     'Starting low and slow... ish. For me. So, not really slow. Shush.');
...
from (select * from recipeTarget where recipe = 'yi_construction-robot_recipe' and craftingName = 'Assembling machine 5 w 5 raw speed 2' and targetName = '1 per Second') r1
...
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Raw speed 2' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
...
and r3.recipe in ('bob-construction-robot-3')
and r4.recipe in ('bob-construction-robot-2')
and r5.recipe in ('construction-robot')
and r6.recipe not in ('flying-robot-frame')
