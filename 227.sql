...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('227',     
                     ...
                     'Time to make those batteries we promised last episode..');
...
from (select * from recipeTarget where recipe = 'silver-zinc-battery' and craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = 'Purple Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Plant MK3 w 5 Raw Speed 2' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Plant MK3 w 5 Raw Speed 2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Plant MK3 w 5 Raw Speed 2' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical Plant MK3 w 5 Raw Speed 2' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
...
where r2.recipe IN ('silver-oxide')
and r3.recipe in ('silver-nitrate')
