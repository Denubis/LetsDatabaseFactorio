...
update episode set uploaddate = '2015-05-23' where episodeid <= 133;
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('136',     
                     'Yuoki ingredient lines' ,
                     'Oh boy, fiddly ingredients.');
...
from (select * from recipeTarget where item = 'alien-science-pack' and craftingName = 'Assembling machine 3' and targetName = 'Yellow Belt 1 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Assembling machine 3' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Assembling machine 3' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
