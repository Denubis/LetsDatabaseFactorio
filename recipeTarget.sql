...
SELECT item,
  ...
  FROM (SELECT recipeProduct.item
             , recipe.recipe
             , craftingMachine.craftingName
             , (recipeProduct.amount/recipe.craftingSpeed) * craftingMachine.craftingSpeed as amountPerSecond
             , (1/recipe.craftingSpeed)*craftingmachine.craftingSpeed as craftingSpeed
        FROM recipe 
        JOIN recipeProduct using (recipe)
           , craftingMachine)
  JOIN recipeComponent USING (recipe)
     , outputTarget
where recipe not like '%barrel%' 
  and recipe not like '%splitter%' 
  and recipe not like '%canister%' 
  and recipe not like '%converter%' 
  and recipe not like 'y-ac%'
  and recipe not like 'n-y-ac%'
     ;
...
from (select * from recipeTarget where recipe in ('angels-plate-copper') and craftingName = 'Casting Machine MK2' and targetName = 'Red Belt 2 side') r1
...
and r2.recipe IN ('liquid-molten-copper')
and r3.recipe IN ('ingot-copper')
)
...
.width 25 100
select item, recipe from recipeTarget where item in ('gas-oxygen') and craftingName = 'Casting Machine MK2' and targetName = 'Red Belt 1 side';
