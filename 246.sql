...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('246',     
                     'Gem barreling',
                     'It turns out that gems are... fiddly');
...
from (select * from recipeTarget where recipe = 'bob-diamond-5' and craftingName = 'Assembling machine 6 w 6 raw speed 2' and targetName = '1 per Second') r1
...
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Crystalizer Mk2 with 2 Raw Speed 2' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
...
where r2.recipe IN ('bob-diamond-4')
and r3.recipe in ('bob-diamond-3')
