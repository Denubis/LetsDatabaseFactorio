...
from (select * from recipeTarget where item = 'solid-carbon' and recipe like '%%' and craftingName = 'Chemical Plant' and targetName = 'Red Belt 1 side') r1
LEFT OUTER JOIN (select * from recipeTarget where recipe not like 'y-ac%' and craftingName = 'Steel furnace' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where recipe not like 'y-ac%' and craftingName = 'Ore Crusher Mk 2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
where r2.item != 'water'
and r3.recipe IN ('coal-crushed')
)
