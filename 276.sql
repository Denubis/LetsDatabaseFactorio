...
-- 1 4x beacon row of 11 for Rocket Fuel

REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('276',     
                     'Rocket Train Depot',
                     'Well, maybe a train-rocket depot? We need to double trains on our network in any event.');
...
from (select 1 as r1productivityratio),
     (select 1 as r2productivityratio),
     (select 1 as r3productivityratio),
(select * from recipeTarget where recipe = 'cellulose-fiber-algae' and craftingName = '37.1' and targetName = 'Purple Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '92.7' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '238.7' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '238.7wa' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '317.1' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '175.7' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '92.7' and targetName = '1 per Second') r7 on (r6.constituentItem = r7.item)

where r3.recipe IN ('gas-carbon-dioxide-from-wood') --IN ('rocket-control-unit')
--and r3.recipe in ('gas-dimethylhydrazine')
--and r4.recipe IN ('gas-dimethylamine')
--and r5.recipe in ('gas-methanol-from-wood')
--and r6.recipe in ('cellulose-fiber-algae')
--and r6.recipe in ('water-separation')
--and r5.recipe in ('salt-water-electrolysis-2')
