...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
	         VALUES('10', 	   'Actually Redoing furnace columns.', 'The quest for basic furnace columns continues.');

SELECT EpisodeID, EpisodeName, Description from episode where episodeid = 10 order by episodeid desc limit 1;	         
...
REPLACE INTO outputTarget(targetName, 			targetAmount) 
	 VALUES 		    ('Yellow Belt 1 side', 	20*1.0/3),
                  ('Molten Copper', 20.0/3),
                  ('Copper Ingot', 8);

.width 30  

-- 5 ore -> 5 ingots. 3 ingots -> 3 molten copper. 2 molten copper -> 2 plates.
...
SELECT item, 
  ...
  FROM (SELECT recipe.item,
	  		   (recipe.amount/recipe.craftingSpeed) * craftingMachine.craftingSpeed as amountPerSecond, (1/recipe.craftingSpeed)*craftingmachine.craftingSpeed as craftingSpeed
