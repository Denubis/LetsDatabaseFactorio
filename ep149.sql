...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('149',     
                     'Circuit reconceptualisation' ,
                     'Current basic circuit designs do not work. Let us reconsider.');
...
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Blast Furnace MK2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical plant MK2' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
...
where r2.recipe IN ('angels-solder-smelting-2')
