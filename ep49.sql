...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('49',     
                     'Yuoki railway locomotive assembly',
                     'Now with steam era coins complete, we can make ... stuff?');
...
REPLACE INTO recipe (item,              amount, craftingSpeed) 
     VALUES      ('Engine unit', 1,      20),
                 ('Iron pipe',       1,      0.5),
                 ('Electric engine unit',       1,      20);

REPLACE INTO recipeComponent (item,        constituentItem, constituentItemAmount)            
   VALUES         ('Engine unit', 'Steel plate',     1),
                  ('Engine unit', 'Iron gear wheel', 1),
                  ('Engine unit', 'Iron pipe',       1),
                  ('Iron pipe',   'Iron plate',     1),
                  ('Inserter',   'Basic circuit board',     1),                  
                  ('Electric engine unit',   'Engine unit',     1),
                  ('Electric engine unit',   'Basic electronic board',     2),
...
select primaryRecipe.item 
  ...
  left outer join recipeTarget as tertiaryRecipe  on (secondaryRecipe.constituentItem = tertiaryRecipe.item)
   where primaryRecipe.item = 'Engine unit'
