...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('226',     
                     'Silver and batteries',
                     'We learned last episode that we will indeed need silver. Best be making it then.');
...
from (select * from recipeTarget where recipe = 'angels-plate-silver' and craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = 'Purple Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 2' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '' and targetName = '1 per Second') r7 on (r6.constituentItem = r7.item)
--where r2.recipe IN ('solid-ammonium-paratungstate-smelting')
--and r3.recipe in ('processed-tungsten-smelting')
