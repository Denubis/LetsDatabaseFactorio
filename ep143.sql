...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('143',     
                     'Expanding basic electronic component production' ,
                     'Oh, this is going to need so much stuff!');
...
from (select * from recipeTarget where item = 'basic-electronic-components' and craftingName = 'Electronics assembling machine 2' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical plant MK2' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Chemical plant MK2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
