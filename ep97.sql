...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('97',     
                     'Finishing basic electronic components.' ,
                     'This turned out to be more fiddly than anticipated. Shock.');
...
from (select * from recipeTarget where item = 'electronic-components' and craftingName = 'Electronics assembling machine 1' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Assembling machine 2' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK2' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Induction Furance MK2' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Blast Furnace MK1' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
...
--where r2.recipe IN ('angels-wire-coil-tin-converting', 'coke-purification')
