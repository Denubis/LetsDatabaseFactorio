...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('183',     
                     'Prototyping Electronic Logic Boards',
                     'It is rather important to automate them at scale but that is not something we are doing today.');
...
from (select * from recipeTarget where recipe = 'invar-alloy' and craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK3 w 2 Speed Module' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '' and targetName = '1 per Second') r7 on (r6.constituentItem = r7.item)
--where r3.recipe IN ('nickel-ore-smelting')
--and r3.recipe in ('sponge-titanium-smelting')
