...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('200',     
                     'Module logic board production',
                     'Episode 200 is... quite similiar to all the prior ones, to be quite honest. Sometimes round numbers aren''t that special.');
...
from (select * from recipeTarget where recipe = 'speed-processor' and craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = '1 per Second') r4 on (r3.constituentItem = r4.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = '1 per Second') r5 on (r4.constituentItem = r5.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Electronics assembling machine 3 w 6 Speed 1' and targetName = '1 per Second') r6 on (r5.constituentItem = r6.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '' and targetName = '1 per Second') r7 on (r6.constituentItem = r7.item)
where r2.recipe IN ('module-processor-board')
--and r3.recipe in ('fibreglass-board')
