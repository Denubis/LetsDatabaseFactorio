...
update episode set uploaddate = '2017-05-07' where episodeid < 122 and uploaddate is null;
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('123',     
                     'More raw ore!' ,
                     'The first OREdr of business is to fix mining.');
...
from (select * from recipeTarget where item = 'battery' and craftingName = 'Assembling machine 3' and targetName = 'Red Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Casting Machine MK2' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = '' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
