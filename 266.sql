...
REPLACE INTO Episode(EpisodeID, EpisodeName, Description) 
              VALUES('266',     
                     'Final bit of debugging solder',
                     'Now to figure out what is wrong with other stations.');
...
from (select * from recipeTarget where recipe = 'angels-plate-silicon' and craftingName = 'Angels MK4 w 2 Raw Speed 8' and targetName = 'Purple Belt 2 side') r1
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 8' and targetName = '1 per Second') r2 on (r1.constituentItem = r2.item)
LEFT OUTER JOIN (select * from recipeTarget where craftingName = 'Angels MK4 w 2 Raw Speed 8' and targetName = '1 per Second') r3 on (r2.constituentItem = r3.item)
...
--where r2.recipe  IN ('angels-solder-smelting-3')
